# ImSwitch Plugin Prompt Reference

When you first run cookiecutter to build a ImSwitch plugin, you will be prompted
...
This is the desired name for your ImSwitch plugin, and will also be the name
...
for user-facing text, ImSwitch and the ImSwitch hub will use the `display_name` (below).
...
This is how ImSwitch discovers plugins on launch.

## display_name

User-facing text to display as the name of this plugin. For example, this will be
used in the ImSwitch plugin menu and in the ImSwitch File > Open Sample menu, as well
as by the ImSwitch hub. It should be 3-40 characters long. It will be listed in
`ImSwitch.yaml` under the `display_name` field.
...
Choosing `"y"` for this prompt will create an example controller implementation
inside your plugin's module in the file `_controller.py`. You can then edit the code in this
file to achieve the reading functionality you want. For more information on
controller see the [ReadTheDocs](https://imswitch.readthedocs.io/en/stable/adding-device-support.html#detectormanager).

## include_manager_plugin

Choosing `"y"` for this prompt will create an example manager hook
implementation inside your plugin's module in the file `_manager.py`. You can
...
[readme](https://github.com/ImSwitch/cookiecutter-ImSwitch-plugin#automatic-deployment-and-version-management)
for details.
